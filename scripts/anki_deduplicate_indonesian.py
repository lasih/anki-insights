...
# Safer default: just tag duplicates instead of deleting them
TAG_DUPLICATES = True
DUPLICATE_TAG = "token_duplicate"

# Export analysis so you can review before doing anything destructive
EXPORT_CSV_PATH = "anki_token_dedup_report.csv"
...
# =========================
# Officially, spaCy supports Indonesian language defaults and blank pipelines,
# which is appropriate when no dedicated pretrained pipeline is available.
nlp = spacy.blank("id")


def normalize_indonesian_token(token_text: str) -> str:
    t = token_text.lower().strip()

    # Remove surrounding punctuation-like chars but keep internal hyphen first
    t = re.sub(r"^[^\w]+|[^\w]+$", "", t, flags=re.UNICODE)
    if not t:
        return ""

    # Numbers are usually not useful for vocabulary novelty
    if t.isdigit():
        return ""
...
def tokenize_indonesian(text: str) -> Set[str]:
    doc = nlp(text)
    out: Set[str] = set()
    ...
    for tok in doc:
        ...
        norm = normalize_indonesian_token(tok.text)
        if not norm:
            continue
...
def analyze_notes(notes: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for idx, note in enumerate(notes, start=1):
        ...
        tokens = tokenize_indonesian(front_text)
...
def main() -> None:
    ...
    print(f"Unique normalized tokens covered: {len(seen_tokens)}")
    print(f"CSV report written to: {EXPORT_CSV_PATH}")
